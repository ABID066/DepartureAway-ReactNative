...
interface ItemProps {
  ...
  title?: string;
  agency?: string;
  personImg?: ImageSourcePropType | string | undefined;
  personOnline?: boolean;
}
        ...
        <View className='flex-row items-center justify-between w-full mt-auto'>
          <View className='flex-row gap-1 items-center'>
            <Text className='text-2xl font-semibold text-[#2B2B2B]'>
              {item?.agency || "Skyward Bliss"}
            </Text>
            <Text
              style={{
                color: item?.personOnline ? "#22c55e" : "#888",
                fontSize: 13,
              }}>
              {item?.personOnline ? "Active now" : "Offline"}
            </Text>
          </View>
        ...
        <>
          ...
          <TouchableOpacity className='flex-row gap-3 bg-white rounded-xl p-2 border border-[#F2F2F2] px-3 py-4 w-[90%] max-w-[90%] overflow-hidden z-10 mx-auto mt-2'>
            ...
            <View className='flex-1'>
              ...
              <Text className='text-[#FF1A5A] font-medium text-xs'>
                Economy
              </Text>
            </View>
          </TouchableOpacity>
