...
const GuiderServices = () => {
  ...
  const { user } = useAuth();
  const { getGuiderServices, getGuiderServicesForUser, deleteGuiderService } =
    useServicePackages();
  ...
  const {
    ...
    queryKey: ["guiderPackages", "services"],
    queryFn: ({ pageParam = 1 }) => getGuiderServices(pageParam, itemsPerPage),
    initialPageParam: 1,
  ...
  const {
    ...
    queryKey: ["userGuiderPackages", user?.id],
    queryFn: () => getGuiderServicesForUser(user?.id),
    enabled: user?.role !== "admin",
  });

  const { mutateAsync: deleteGuiderServiceMutation, reset } = useMutation({
    mutationKey: ["guider-services", "services"],
    mutationFn: async (id: string) => await deleteGuiderService(id),
    onSuccess: async () => {
      Toast.show({
        type: "success",
        text1: "Guider Service Deleted Successfully",
        position: "top",
      });
      reset();
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteGuiderServiceMutation(selectedId);
    } catch (error) {
      console.error("Error deleting guider package:", error);
      Toast.show({
        type: "error",
        text1: "Failed to delete guider service",
        position: "top",
      });
    } finally {
  ...
  return (
    <View className='flex-1 bg-white p-4'>
      <View className='flex-row justify-between items-center my-6'>
        <Text className='text-2xl font-bold'>Guider Services</Text>
        <Link href='/dashboard/services/guider-services/create-new' asChild>
          <TouchableOpacity className='bg-[#FF1A5A] px-4 py-2 rounded-lg'>
            <Text className='text-white font-medium'>
              + Create New Guider Service
            </Text>
          </TouchableOpacity>
        </Link>
      </View>
        ...
        <>
          <ScrollView
            ...
            <View className='min-w-[1200px]'>
              ...
              <ScrollView>
                {currentPageData?.map(
                  (service: GuiderServiceData, index: number) => (
                    <View
                      key={index}
                      ...
                      <View className='w-[15%] px-4 py-3 flex-row gap-4'>
                        <Link
                          href={`/dashboard/services/guider-services/update/${service?._id}`}
                          asChild>
