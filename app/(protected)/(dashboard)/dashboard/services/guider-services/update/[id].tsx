...
const UpdateGuiderService = () => {
  const { id } = useLocalSearchParams();
  const router = useRouter();
  const { getSingleGuiderService, updateGuiderService } = useServicePackages();
  ...
  const { data, isLoading: isFetching } = useQuery({
    queryKey: ["guiderService", "services", id],
    queryFn: async () => await getSingleGuiderService(id as string),
    enabled: !!id,
  });
  const guiderService: GuiderServiceData = data?.data;
  ...
  const toggleLanguage = (language: string) => {
    if (tempSelectedLanguages.includes(language)) {
      setTempSelectedLanguages(
        tempSelectedLanguages.filter((lang) => lang !== language)
      );
    } else {
  ...
  useEffect(() => {
    if (guiderService) {
      setTempSelectedLanguages(guiderService.languages || []);
      reset({
        name: guiderService.name || "",
        bio: guiderService.bio || "",
        languages: guiderService.languages || [],
        location: guiderService.location || "",
        experience: guiderService.experience || "",
        specialty: guiderService.specialty || "",
        hourlyRate: guiderService.hourlyRate
          ? String(guiderService.hourlyRate)
          : "",
        dailyRate: guiderService.dailyRate
          ? String(guiderService.dailyRate)
          : "",
        isVerified: guiderService.isVerified || false,
        available: guiderService.available,
        contactInfo:
          typeof guiderService.contactInfo === "string"
            ? guiderService.contactInfo
            : guiderService.contactInfo?.email ||
              guiderService.contactInfo?.phone ||
              "",
        images: guiderService.imageUrl || [],
      });
    }
  }, [guiderService, reset, id]);

  // Update mutation
  const { mutateAsync } = useMutation({
    mutationFn: async (serviceData: GuiderServiceData) => {
      const { data } = await updateGuiderService(id as string, serviceData);
      return data;
    },
    onError: () => {
      Toast.show({
    ...
    mutationKey: ["update-guider-service", "services", "service", id],
    onSuccess: () => {
      Toast.show({
        type: "success",
  ...
  return (
    <ScrollView className='flex-1 bg-white p-4'>
      ...
      {errors.specialty && (
        <Text className='text-red-500 mb-2'>{errors.specialty.message}</Text>
      )}
      ...
      {errors.languages && (
        <Text className='text-red-500 mb-2'>{errors.languages.message}</Text>
      )}
      ...
      />
      {errors.experience && (
        <Text className='text-red-500 mb-2'>{errors.experience.message}</Text>
      )}
      ...
      />
      {errors.hourlyRate && (
        <Text className='text-red-500 mb-2'>{errors.hourlyRate.message}</Text>
      )}
      ...
      />
      {errors.dailyRate && (
        <Text className='text-red-500 mb-2'>{errors.dailyRate.message}</Text>
      )}
      ...
      />
      {errors.contactInfo && (
        <Text className='text-red-500 mb-2'>{errors.contactInfo.message}</Text>
      )}
...
export default UpdateGuiderService;
