...
const ExclusiveOfferDetails = () => {
  ...
  const { getServiceById } = useServicePackages();
  const [ServiceClass, setServiceClass] = useState("Basic Package");

  const isBasic = ServiceClass === "Basic Package";
  const isStandard = ServiceClass === "Standard Package";

  const { data, status } = useQuery({
    queryKey: ["offerDetails", "services", id],
    queryFn: async () => await getServiceById(id as string),
    enabled: !!id,
  });
  
  const service: ServiceData | undefined = data?.data;
  ...
  if (!service) {
    return (
      <View className='flex-1 justify-center items-center'>
        <Text>No service data found</Text>
      </View>
    );
  }
        ...
        {status === "success" && (
          <>
            <View className='flex-row gap-1 items-center my-4 border-t border-[#F2F2F2] pt-4'>
              <TouchableOpacity
                onPress={() => setServiceClass("Basic Package")}
                className={`${
                ...
                <Text
                  ...
                  Basic Package (${service.price_basic})
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => setServiceClass("Standard Package")}
                className={`${
                ...
                <Text
                  ...
                  Standard Package (${service.price_standard})
                </Text>
              </TouchableOpacity>
            </View>
            ...
            <View className='py-4 gap-2 mb-6'>
              <View className='flex-row items-center gap-2 mb-2'>
                <Image
                  source={
                    service.provider_id?.image
                      ? { uri: service.provider_id.image }
                      : images.ellipse
                  }
                  className='w-8 h-8 rounded-full'
                />
                <Text className='font-medium text-lg text-[#4F4F4F]'>
                  {service.provider_id?.name || "Skyward Bliss"}
                </Text>
              </View>
        ...
        <View className='flex-row gap-3'>
          <Link
            href={{
              ...
              params: {
                ...
                tourPackagePrice: isBasic
                  ? service.price_basic.toString()
                  : service.price_standard.toString(),
              },
            }}
            asChild>
          ...
          <Link
            href={{
              ...
              params: {
                id: service.provider_id?.id || service.provider_id?._id || "",
                name: service.provider_id?.name || "",
                image: service.provider_id?.image || "",
              },
            }}
            asChild>
