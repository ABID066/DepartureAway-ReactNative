...
const categories: { id: number; name: string; icon: keyof typeof Ionicons.glyphMap }[] = [
  { id: 1, name: 'Flight', icon: 'airplane' },
  { id: 2, name: 'Hotel', icon: 'business' },
    ...
    <View className="px-4 -mt-6">
      <View className="flex-row justify-around">
        {topCategories.map((category) => (
          <TouchableOpacity 
            key={category.id} 
            className="bg-white rounded-xl shadow w-16 h-16 items-center justify-center mb-4"
          >
            <View className="items-center">
              <Ionicons 
                ...
                color="#1A3F85" 
              />
              <Text className="text-xs text-blue-900 mt-1">{category.name}</Text>
            </View>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <View className="flex-row justify-center">
        {bottomCategories.map((category) => (
          <TouchableOpacity 
            key={category.id} 
            className="bg-white rounded-xl shadow w-16 h-16 items-center justify-center mx-4"
          >
            <View className="items-center">
              <Ionicons 
                name={category.icon} 
