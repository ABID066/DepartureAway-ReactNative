...
const DiscoverPlacesSection = ({ 
  ...
  const getDataAndRender = () => {
    if (activeTab === 'For you') {
      return {
        data: popularPlaces,
        renderItem: (item: PlaceItem, index: number) => renderPlaceCard(item, index)
      };
      ...
      return {
        data: countryData,
        renderItem: (item: CountryItem, index: number) => renderCountryCard(item, index)
      };
    }
  };
    ...
    >
      ...
      <View className="absolute bottom-0 left-0 right-0 p-2">
        ...
        <View className="flex-row items-center">
          ...
          <Text className="text-xs text-white ml-1" numberOfLines={2}>{item.location}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
    ...
    >
      <View className="flex-row items-center mb-2">
        <Image 
          ...
          resizeMode="cover"
        />
        <Text className="text-lg font-bold text-gray-800 ml-2">{item.name}</Text>
      </View>
  ...
  return (
    <View className="mt-5 px-4">
      <View className="flex-row justify-between items-center mb-4">
        ...
        <TouchableOpacity>
          <Text className="text-md border-rose-500 border rounded-2xl px-2 py-1 text-rose-500 font-semibold">See all</Text>
        </TouchableOpacity>
      </View>
      ...
      <View className="flex-row mb-4">
        {tabs.map((tab) => (
          <TouchableOpacity
            key={tab}
            className={`mr-5 py-1 ${activeTab === tab ? 'bg-rose-500 px-4 rounded-full' : ''}`}
            onPress={() => setActiveTab(tab)}
          >
            <Text 
              className={`text-base ${activeTab === tab ? 'text-white font-semibold' : 'text-gray-500'}`}
            >
              {tab}
            </Text>
          </TouchableOpacity>
